...
public class Main {
    ...
    Объект, из которого считываются данные - это поток ввода, а объект в который записываются данные - поток вывода.

   Все классы, работающие с вводом-выводом делятся на две большие группы - классы работающие с байтами и классы, работающие
   с символами
   В основе всех классов, работающих с байтами лежат два абстрактных класса - InputStream и OutputStream, а классов,
   которые работают с символами - Reader, Writer
    ...
    available() - возвращают количество байтов, доступных для чтения в потоке
...
//                System.out.print((char) i);
        ...
        //  чтение и запись
        try(FileInputStream fileInputStream = new FileInputStream("output.txt");FileOutputStream fileOutputStream = new FileOutputStream("newOutput.txt")) {
            byte[] buffer = new byte[512];
            int counter;
